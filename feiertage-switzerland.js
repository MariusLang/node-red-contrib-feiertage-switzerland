...
module.exports = function (RED) {
  function feiertageCH(config) {
    ...
    const checkPeterUndPaul = config.peterUndPaul; // checkbox Peter und Paul
    const peterUndPaulName = config.peterUndPaulName; // Peter und Paul Name
    ...
    function checkbox() {
      ...
      if (checkPeterUndPaul) {
        ...
        const index = holiday.indexOf(peterUndPaul); // get index of item
        if (index >= 0) {
          holiday.splice(index); // remove item at index
        }
      }
