module.exports = function(RED) {
    function feiertageCH(config) {
        ...
        let checkBerchtoldstag = config.berchtoldstag; // checkbox Berchtoldstag
        let berchtoldstagName = config.berchtoldstagName; // Berchtoldstag Name
        ...
        let checkPeterUndPaul = config.peterUndPaul; // checkbox Peter und Paul
        let peterUndPaulName = config.peterUndPaulName; // Peter und Paul Name
        let checkBundesfeierCH = config.bundesfeierCH; // checkbox Bundesfeier
        let bundesfeierCHName = config.bundesfeierCHName; // Bundesfeier CH Name
        ...
        const peterUndPaul = {
            id: "Peter und Paul",
            name: peterUndPaulName,
            dateObj: Feiertage.getPeterUndPaul(Feiertage.formatDateObj, currentYear),
            dateDE: Feiertage.getPeterUndPaul(Feiertage.formatDE, currentYear)
        }
        const bundesfeierCH = {
        ...
        function refreshHoliday() {
            ...
            if ((new Date(berchtoldstag.dateObj) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                berchtoldstag.dateObj = Feiertage.getBerchtoldstag(Feiertage.formatDateObj, currentYear + 1);
                berchtoldstag.dateDE = Feiertage.getBerchtoldstag(Feiertage.formatDE, currentYear + 1);
            }
            ...
            if ((new Date(stJosef.dateObj) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                stJosef.dateObj = Feiertage.getStJosef(Feiertage.formatDateObj, currentYear + 1);
                stJosef.dateDE = Feiertage.getStJosef(Feiertage.formatDE, currentYear + 1);
            }
            ...
            if ((new Date(peterUndPaul.dateObj) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                peterUndPaul.dateObj = Feiertage.getPeterUndPaul(Feiertage.formatDateObj, currentYear + 1);
                peterUndPaul.dateDE = Feiertage.getPeterUndPaul(Feiertage.formatDE, currentYear + 1);
            }
            if ((new Date(bundesfeierCH.dateObj) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                bundesfeierCH.dateObj = Feiertage.getBundesfeierCH(Feiertage.formatDateObj, currentYear + 1);
                bundesfeierCH.dateDE = Feiertage.getBundesfeierCH(Feiertage.formatDE, currentYear + 1);
            }
            ...
            if ((new Date(mariaeEmpfaengnis.dateObj) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                mariaeEmpfaengnis.dateObj = Feiertage.getMariaeEmpfaengnis(Feiertage.formatDateObj, currentYear + 1);
                mariaeEmpfaengnis.dateDE = Feiertage.getMariaeEmpfaengnis(Feiertage.formatDE, currentYear + 1);
            }
        ...
        function checkbox() {
            ...
            if (checkBerchtoldstag) {
                ...
                let index = holiday.indexOf(berchtoldstag); // get index of item
                if (index >= 0) {
                    holiday.splice(index); // remove item at index
                }
            }
            ...
            // check Peter und Paul is activated
            if (checkPeterUndPaul) {
                holiday.push(peterUndPaul); // add Peter und Paul to holiday array
            } else {
                let index = holiday.indexOf(peterUndPaul); // get index of item
                if (index >= 0) {
                    holiday.splice(index); // remove item at index
                }
            }
            ...
            if (checkBundesfeierCH) {
                ...
                let index = holiday.indexOf(bundesfeierCH); // get index of item
                if (index >= 0) {
                    holiday.splice(index); // remove item at index
                }
            }
    ...
    RED.nodes.registerType("feiertageCH", feiertageCH);
};